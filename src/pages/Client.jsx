...
import { getClientForms, getUserNameLink } from '../services/api.js';
...
const videoId = getYouTubeVideoId(client.url);
  ...
  return (
    <div className="max-w-xl mx-auto p-4 mb-15">
      ...
      <div class="relative w-full">
        ...
        <input type="text" class="bg-white border-2 border-gray-200 text-gray-500 text-sm rounded-lg focus:outline-lime-700 block w-full ps-10 p-4.5 " placeholder="Search link -or- number"
          value={search}
          onChange={(e) => setSearch(e.target.value)} />
    </div>
    <button class="p-2 bg-red-100 text-sm font-medium text-red-600 border-red-200  rounded-lg  border-2  focus:outline-none ">
        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#ef4444"><path d="M320-240 80-480l240-240 57 57-184 184 183 183-56 56Zm320 0-57-57 184-184-183-183 56-56 240 240-240 240Z"/></svg>
    </button>
    </div>
     <div className='flex py-4 gap-4 items-center max-w-2xl mx-auto'>
      <div className=''>
        <img src={profile.profilePic} alt="" className='w-20 h-full shadow-sm rounded-full'/>
      </div>
      <div>
        <div className='text-xl text-neutral-800 font-display capitalize'>{profile.username} <img className='h-3 inline' src={profile.plan=='pro' || profile.plan=='premium'?'/src/assets/verify.png':''} alt="" /></div>
    ...
    <div  className=' shadow-[1px_50px_100px_9px_rgba(234,_179,_8,_0.5)] fixed flex items-center justify-evenly gap-1 bg-linear-65 from-amber-400 to-amber-500 bottom-10 translate-x-1 border-1 px-6 py-3  text-white text-xl rounded-full outline-amber-300 outline-2 active:translate-1 hover:bg-linear-65 hover:from-amber-500 hover:to-amber-400 transition-all ease-in-out duration-700'>
      <span className='font-stretch-105% text-xs '>Create your free mappi</span>
    ...
    <hr className='h-0.25 bg-gray-100 my-4 border-none ' />

    <div className='flex justify-between'><p className='my-3 text-xl text-emerald-600' >All Links</p> <button className='flex items-center text-[#D62F2F] capitalize text-xl' onClick={()=> tab=="links" ? setTab('forms'):setTab('links')}>{tab=='links'?'forms':'links'}<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#D62F2F"><path d="m560-240-56-58 142-142H160v-80h486L504-662l56-58 240 240-240 240Z"/></svg></button> </div>
            ...
            <div className="bg-white w-full border border-gray-200 rounded-xl p-4 shadow-2xs hover:shadow-sm transition ease-in">
              {videoId ? (
                <>
                  ...
                  />
                  <span className="inline-block text-red-600 font-semibold rounded ml-3 p-2 mt-2 capitalize">
                    {link.title}
                    ...
                    {link.logo && (
                      <div>
                        <img
                          src={link.logo}
                          alt="logo"
                          className="w-full h-14 object-contain rounded shrink contain"
                        />
                      </div>
                    )}
                    <div>
  ...
  {forms.map((res)=>(
    <div onClick={()=>navigate(`/form/${res._id}`)} className='p-4 border-1 rounded-xl border-gray-200 flex justify-between'>
